...
#                 continueSearch = False
            for table in tables:
                ...
                #print(f'headerMatch:{headerMatch}')
                if headerMatch>0:
...
endpoint_column_lst = {'objectives','endpoints','enppoints'}
...
def matchObjectiveEndpointTable(table):
    ...
    oe_match = endpoint_column_lst & set(first_row)
    return oe_match
...
def SOAfromResponseUsingPA(response,tabletype,jsontype=False,pretty=False):
    ...
    for page in pagelist:
        ...
        ### checkPA #########
        pa_match = matchProtocolActivityTable(combinetable)
        if len(pa_match)>2:
            if tabletype == 'csv':
                outputtable = ''
                for r in combinetable:
                ...
                outputtable = combinetable     
            output[tables_count] = {'table':outputtable,'pages':pagefound,'protocol_matched':list(pa_match)}
            tables_count+=1
        #####################
    ...
    for page in pagelist:
        ...
        if len(oe_match)>1:
            if tabletype == 'csv':
                ...
                outputtable = combinetable     
            output[tables_count] = {'table':outputtable,'pages':pagefound,'columns_matched':list(oe_match)}
            tables_count+=1
        #####################
...
#             return response
#     return None
# path = 'data/processed/textract/document/pfizer/phase1/'
# filename = 'NCT03865446.json' #'NCT02841267.json' NCT03077607 #NCT02129205 NCT02222922 NCT02367456 NCT02841267
# response = loadresponse(filename)
# #output = SOAfromResponse(response,jsontype=False,pretty=False,tabletype='html')
# tabletype = 'html' #'html' 'csv'

# # output = SOAfromResponseUsingPA(response,jsontype=True,pretty=False,tabletype=tabletype)
# # print(output)

# output = endpointtablefromResponse(response,jsontype=True,pretty=False,tabletype=tabletype)
# print(output)

# path_output = 'data/processed/SOA/pfizer/'
# f = open(f'{path_output}soa_{tabletype}_{filename}', "w")
# f.write(output)
# f.close()

# for page in output:
#     print(f'SOA search found on page:{page}')
#     tablespan = ' '.join(str(x) for x in output[page]["pages"])
#     print('Tablespan: '+tablespan)
#     print('---------------')
#     print(output[page]['table'])
#     print('_______________')

# # ### Inference ###
# #s3 
# s3BucketName = "iso-clinicaltrial-studyprotocols"
# documentPath = "study_protocols_pfizer_phase1/"
# #document
# filename = 'NCT02129205.pdf'

# ### Inferance call ###
# response = get_textract_response(s3BucketName,documentPath,filename) #This response is the document analysis result from textract
# print(SOAfromResponse(response,jsontype=True,pretty=False,tabletype='csv'))