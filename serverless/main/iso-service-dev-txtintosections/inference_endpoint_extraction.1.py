...
endpoint_column_lst = {'objective','endpoint'} #,'enppoint'}
...
def selectTableusingTOC(tableOutput,sectionNames):
...
def nctExtractObjectivesEndpoints(response,text,tabletype):
    ...
    if tableOutput:
        output['type'] = 'table'
        output['content'] = selectTableusingTOC(tableOutput,sectionNames) #tableOutput[0]
    ...
    #Else extract the text instead:
                ...
                for r in _result:
                    if 'name' in _result[r]:
                        if _result[r]["name"] not in found_sections:
                            ...
                            print(_result[r])
                            if 'rawtext' in _result[r]:
                                rawtext = _result[r]['rawtext']
                            else:
                                rawtext = _result[r]['text']
        ...
        # output[0] = {}
        # output[0]['text'] = text
        # output[0]['found_sections'] = found_sections
    return output
