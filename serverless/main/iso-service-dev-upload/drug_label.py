...
class DrugLabel(object):
    def __init__(self,xmlstr):
        ...
        self.tree = ET.ElementTree(ET.fromstring(xmlstr))
        self.tree_et = etree.ElementTree(etree.fromstring(xmlstr))
    ...
    def process(self):
        ...
        try:
            ...
            section_text = self.extract_text_sections()
    ...
    def extract_summary(self):
        ...
        metadata["revisionDate"] = effectiveTime
        ...
        # active ingredients
        substance_name = sorted([self.strip_newline_tab(a.text) for a in self.tree_et.xpath(".//.//manufacturedProduct//activeMoiety/activeMoiety/name")]) 
        substance_name = ", ".join(set(substance_name))
        metadata["substanceName"] = substance_name

        ## inactive ingredients
        inactive_ingredients = sorted([self.strip_newline_tab(inactive.text) for inactive in self.tree_et.xpath(".//manufacturedProduct//inactiveIngredient/inactiveIngredientSubstance/name")])
        ...
        ## other ingredients
        ingredients = sorted([self.strip_newline_tab(ingredient.text) for ingredient in self.tree_et.xpath(".//manufacturedProduct//ingredient/ingredientSubstance/name")])
        ...
        # consumed in
        consumed_in =  self.tree_et.xpath(".//manufacturedProduct//consumedIn/substanceAdministration/routeCode/@displayName")
    ...
    def extract_text_sections(self):
        ...
        for index, sec in enumerate(sections):
            sec_name = (self.strip_newline_tab(sec.xpath(".//@displayName")[0])) if len(sec.xpath(".//@displayName"))>0 and sec.xpath(".//@displayName")!=None else f"section{index}"
            
            response = response + sec_name
    ...
    def __recursively_get_text(self,sec):
        ...
        for compsec in component_sections:
            ...
            if len(title) > 0:
                ...
                title = ""
            response+= (title + "\n " + content  + "\n " + self.__recursively_get_text(compsec))
        return response   
