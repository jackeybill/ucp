...
def getExistResult(s3BucketName, documentName):
    ...
    pages = []
    try:
        outputName = documentName.replace('RawDocuments','TextractOutput')
        response = s3_client.get_object(Bucket=s3BucketName, Key=outputName+".json")
        pages = json.loads(response['Body'].read().decode('utf-8'))
...
def lambda_handler(event, context):
    ...
    #pages = getTextractPagesResult('c8301f8f1d376ce154f53b37696e3d3cd02f52d6158a61b62009484e4c2b9f4f')
    #print(pages)
    if 'urlOpen' in event:
        url = quote(event['urlOpen'], safe = string.printable)
    ...
    try:
        pages = getExistResult(s3BucketName, documentName)
        logger.info("len of pages {}".format(len(pages)))
